...
Reads from D:\S1\_question_bank\FINAL_MEGA_BANK.json
"""
...
try:
    import sys as _sys
    _sys.path.insert(0, r"G:\lam-fung-academy")
    from lf_ai_brain import ai_semantic_mark
    _AI_AVAILABLE = True
except Exception:
    _AI_AVAILABLE = False
    def ai_semantic_mark(*a, **kw): return {"status": "REVIEW", "confidence": 0.0, "reason": "AI unavailable", "_ai_used": False}
...
def mark_answer(student_answer: str, question_id: int) -> MarkResult:
    ...
    # -- Level 5: AI Semantic Comparison (frellmapi) --
    if _AI_AVAILABLE:
        question_text = ""
        try:
            qdata = _get_question_data(question_id)
            question_text = qdata.get("question", qdata.get("text", ""))
        except:
            pass
        ai_result = ai_semantic_mark(s_text, m_text, question_text, max_marks)
        if ai_result.get("_ai_used"):
            status = ai_result.get("status", "REVIEW")
            confidence = ai_result.get("confidence", 0.5)
            if status == "CORRECT":
                return MarkResult(
                    score=max_marks, max_score=max_marks, confidence=confidence,
                    details={'level': 5, 'method': 'ai_semantic', 'ai_status': status, 'reason': ai_result.get('reason','')},
                    status='CORRECT'
                )
            elif status == "PARTIAL":
                partial_score = max(1, int(max_marks * 0.5))
                return MarkResult(
                    score=partial_score, max_score=max_marks, confidence=confidence,
                    details={'level': 5, 'method': 'ai_semantic', 'ai_status': status, 'reason': ai_result.get('reason','')},
                    status='PARTIAL'
                )
            elif status == "WRONG":
                return MarkResult(
                    score=0, max_score=max_marks, confidence=confidence,
                    details={'level': 5, 'method': 'ai_semantic', 'ai_status': status, 'reason': ai_result.get('reason','')},
                    status='WRONG'
                )
